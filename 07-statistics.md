...
A = "Elvis an identical twin"
B = "Evlis is a twin"

Pr(identical | twin) = Pr(twin | identical) * Pr(identical) / (Pr(twin | identical) * Pr(fraternal) + Pr(twin | fraternal) * Pr(fraternal)) 
= (1 * 1/300) / ((1 * 1/300) + (1 * 1/125)
= 0.2941 

Would be even better to have stats on differential prob of death at birth and stats about probability of being a twin for Elvis's ethnic background.
...
Frequentists take at face value the frequencies of the events observed, and treat them as indications of the probabilities of those events.
Bayesians ask what is the probability of observing the data we have given our priors, and then update the probabilities taking into account the priors.
